...
def data_transform ():
    df = (pd.read_csv ('DataHackFI/Capstone_Project/Data/df_enc_2.csv'))
    # df = clean_dataset(df)
...
def cat_boost ():
    ...
    cb_model = CatBoostClassifier (iterations = 1800,
    ...
    df_test_values_trf = pd.read_csv ('C:/Users/OAjayi/PycharmProjects/Datahack4FI/DataHackFI/Capstone_Project/Data'
                                      '/df_test_enc_2.csv')
    ...
    L_prediccion.to_csv('C:/Users/OAjayi/PycharmProjects/Datahack4FI/DataHackFI/Capstone_Project/Data/submission_1.csv')
...
def xgb_classifier (X_train, X_test, y_train, y_test, useTrainCV=False, cv_folds=5, early_stopping_rounds=50):
    ...
    alg = xgb.XGBClassifier (learning_rate = 0.1095, n_estimators = 856, max_depth = 50,
                             ...
                             objective = 'binary:logistic', random_state = 54321, nthread = 4, scale_pos_weight = 1,
                             seed = 27, cv = 5)
...
def xgb_regressor ():
    ...
    crf = GridSearchCV (xgb_model,
                        {'max_depth': [ 6, 8 ], 'n_estimators': [ 500, 855 ], 'learning_rate': [ 0.1, 0.1095 ]},
                        verbose = 1)
    crf.fit (X, y)
...
if __name__ == "__main__":
    ...
    #                              seed = 3456)
    #
    # cb_model = cat.CatBoostClassifier (iterations = 1375,
    #                                    learning_rate = 0.109499,
    #                                    depth = 6,
    #                                    thread_count = 10,
    #                                    eval_metric = 'AUC',
    ...
    #           "rf": rf_model, "cat": cb_model, "avg_ensemble": voting_clf
    #           }
    #
    ...
    # first_stack.fit (X_train, y_train, eval_metric = 'auc', eval_set = eval_set,
    #                  cat_features = categorical_features_pos, verbose = True)
    # # Fit the meta learner
    # second_stack = logreg_clf.fit (meta_features, y_train, eval_metric = 'auc', eval_set = eval_set,
    #                                cat_features = categorical_features_pos, verbose = True)
    #
